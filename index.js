...
var layerback = new CG.Layer('layerback')
...
//screen4 follower/collision/attach demo
...
//        .setEmitterPosition(new CG.Point(50, 50))
    ...
    run1 = new CG.Animation(Game.asset.getImageByName('hunter'), new CG.Point(280, Game.height2), 1, 7)
    run1.cutwidth = 56
    run1.cutheight = 64
    run1.name = 'runner1'
    run1.delay = 6
    ...
    run2 = new CG.Animation(Game.asset.getImageByName('hunter'), new CG.Point(360, Game.height2), 8, 14)
    run2.cutwidth = 56
    run2.cutheight = 64
    run2.name = 'runner2'
    run2.delay = 6
    ...
    run3 = new CG.Animation(Game.asset.getImageByName('hunter'), new CG.Point(440, Game.height2), 16, 24)
    run3.cutwidth = 56
    run3.cutheight = 64
    run3.name = 'runner3'
    run3.delay = 6
    ...
    run4 = new CG.Animation(Game.asset.getImageByName('hunter'), new CG.Point(200, Game.height2), 31, 47)
    run4.cutwidth = 56
    run4.cutheight = 64
    run4.name = 'runner3'
    run4.delay = 6
...
//        .setEmitterPosition(new CG.Point(160, 120))
    ...
    rainbow = new CG.Sprite(Game.asset.getImageByName('rainbow'), 125, 200)
    ...
    for(i=0;i<10;i++){
        cloud = new CG.Sprite(Game.asset.getImageByName('cloud'), 300 + (10 * i), 200 + (5 * i) )
        cloud.name = 'cloud'
        cloud.alpha = 0.4
        cloud.xscale = 0.5
        cloud.yscale = 0.5
...
function clicked() {
    if(Game.director.getActiveScreenName() == 'bitmap'){
        ...
        shoot.play()
        if (mousex % 2) {
            expl = new CG.Animation(Game.asset.getImageByName('exp' + (Math.floor((Math.random() * 5)) + 1)), new CG.Point(mousex, mousey), 1, 16)
            expl.cutwidth = 64
            expl.cutheight = 64
            expl.yspeed = -2
        } else {
            //expl = new CG.Animation('media/img/expbig1.png', mousex, mousey, 1, 64)
            expl = new CG.Animation(Game.asset.getImageByName('burst'), new CG.Point(mousex, mousey), 1, 256)
            expl.cutwidth = 256
            expl.cutheight = 256
            expl.yspeed = -1
        }
